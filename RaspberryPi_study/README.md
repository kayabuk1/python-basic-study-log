...
*   **Languages:** Python 3, HTML/CSS, PHP, Bash
*   **OS & Infrastructure:** Linux (Raspberry Pi OS Bookworm), VNC, SSH
*   **Libraries/Frameworks:** Flask, Jinja2, BeautifulSoup4, RPi.GPIO, smbus2, urllib, re, subprocess
...
*   `Jinja2` テンプレートエンジンを利用し、Pythonで取得したセンサーの値を動的にHTMLへ組み込んでWeb画面に表示する処理の実装練習。
*   `subprocess` モジュールを使用し、カメラストリーミング (`mjpg-streamer`) や音声合成 (`AquesTalkPi`) のコマンドをPythonから呼び出す処理を実装しています。
*   `smbus2` を用いて温湿度センサー (DHT20) から生のバイナリデータを取得し、ビット演算によって数値を復元する処理を実装しました。

### 2. Webスクレイピングの学習記録
BeautifulSoup4と正規表現(`re`)などを組み合わせたデータ抽出のプログラム群です。
*   HTMLのツリー構造から特定のタグや属性(`href`, `src`)を抽出する処理を実装しました。
*   取得先のサーバーに負荷をかけないための待機処理（`time.sleep`）や、User-Agentの記述など、相手方へ配慮した設計を心掛けています。
