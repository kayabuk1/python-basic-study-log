...
pprint(jsondata)
#●↓取得したデータを用いてグラフを作る。グラフを作りにはデータフレームを作る必要があり、
#    データフレームを作るには最初に空のデータフレームを作る必要がある。
#    columns名が気温だけで中身データの無いデータフレームを作る記述。
...
plt.ylim(-10,40)
plt.grid()
plt.savefig(F"{now}_{city}_tempGraph.jpg")
