...
2.ジェネレータ ＝ 「データの一時停止と再開」 現場監督「この部屋でデータを
　作ったら、一旦作業を止めて（yield）**外の呼び出し元にデータを渡すぞ
...
R'''◆そもそもawaitとは何か？
await は関数でもメソッドでもなく「Await式（Await Expression）」 という構文として定義されている。
「awaitable（待機可能）オブジェクトでの coroutine 実行を一時停止します。
  coroutine function 内でのみ使用できます。」
用語：
1.Await式 (await): 
    右側に置かれたオブジェクトの処理が終わるまで「現在のコルーチンの実行を一時停止(サスペンド）させる」為の演算子。
2.待機可能オブジェクト(Awaitable Object): 
    await の右側に置くことが許されている特殊なオブジェクトです。
    具体的には、他のコルーチン関数を呼び出した結果や、asyncio.sleep()のように内部に
    __await__() という特殊なメソッドを持っているオブジェクトのことです。
3.一時停止 (Suspend):
    OSのフリーズ（フリーズ＝CPUを掴んだまま止まる)とは異なり、
    ローカル変数の状態や現在の実行位置をメモリに「記憶」したまま、
    処理の主導権をイベントループ（大元締め）に返却することを指します。

【比喩】 現場監督と作業員のスローモーション
この「一時停止（主導権の返還）」を、インフラ構築の現場に例えてみましょう。
作業員Aがルーター1にログインし、設定保存のコマンド（write memory）を打ち込みました。
ルーターが保存を完了してプロンプトを返してくるまで、**「5秒間の通信待ち（I/O待ち）」**が発生します。
ここで作業員Aが発動するのが await（報告と主導権の返還） です。
作業員A (await 発動！)「現場監督！ 今ルーター1にコマンドを投げました！
    応答が来るまで私の手元は**『5秒間の手待ち状態（Awaitable）』**になります！
    画面を見つめたままボーッと立っているのはCPUの無駄遣いなので、
    一旦この作業を『保留（await）』にして、私の実行権限を監督にお返しします！
    私の時間を他の仕事に回してください！」
現場監督 「よし分かった！ お前の状態（どこまで作業したか）はセーブしておく！
    では、空いたCPU時間を使って、作業員B、お前はルーター2の作業を進めろ！」
(5秒後、ルーター1から完了の応答がPythonに届く)
現場監督 「おっ、ルーター1から結果が返ってきたな！
    作業員A、お前を呼び戻す（レジューム）！さっきセーブした await の地点から作業を再開し、
    ルーターからの返答（戻り値）を受け取って、次の行の処理に進め！」
これが await のメンタルモデルです。
--------------------------------------------------------------------------------
Q.なぜ await は「特定の処理」にしか付けられないのか？
インフラ自動化で「じゃあ、時間を食う重い処理には全部 await を付ければ並行処理できるのでは？」
と思うかもしれません。しかし、普通の time.sleep(5) や requests.get() に await を付けると
エラー（TypeError: object is not awaitable） になります。
なぜか？ それは、time.sleep() が 「現場監督に何も報告せずに、自分勝手に5秒間昼寝を始める
（OSレベルでスレッドをブロックする）関数」 だからです。監督は彼がいつ起きるか分からないため、
他の作業員に仕事を振ることができず、システム全体がフリーズします。
一方、await asyncio.sleep(5) は、「私は5秒間待つことになりますよ、と現場監督に報告して、
正しく主導権を返す機能（__await__() メソッド）を持っている証明書（Awaitable）」 です。
つまり、await はただの「待て」という命令ではなく「私は今からI/O待ちに入ります。
他のタスクを進めてください」という、イベントループに対する『協調のサイン（主導権のパス）』なのです。
'''
