...
class method-wrapper(object)
 ...
 |  
 |  __text_signature__
 Pythonのすべてのオブジェクトは、自身を初期化するための __init__ という特殊メソッド（C言語で作られた組み込み機能）を持っています
。
つまり、mypack.__init__ と書いたとき、現場監督はファイルシステムの __init__.py ではなく、**「mypack というモジュールオブジェクトをメモリ上で構築するための、C言語レベルの初期化機能（メソッド）」**を見つけてきてしまったのです！ ファイル名とオブジェクトの機能名がたまたま同じ __init__ であったために起きた、高度な錯覚
 '''
...
読み込んで実行します。 もし __init__.py の中に from mypack import app 
...
#Q.__main__.pyif __name__=='__main__':判定のパッケージ版とは？どういう意味汗
'''
...
:7: RuntimeWarning: coroutine 'heavy_process' was never awaited
  print(heavy_process('hoge', 5))
