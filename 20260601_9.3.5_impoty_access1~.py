...
動き方（プリエンプティブ）: OSのディスパッチャは、各プログラム（スレッド）に対して「はい、君の持ち時間はここまで！次！」と、**強制的にCPUの主導権を剥奪（コンテキストスイッチ）**します。プログラムの都合はお構いなしです
。
② Pythonのイベントループ（アプリケーションレベル）
権限: OSから与えられた「1つのスレッド（1つのCPU枠）」の中だけで仕切る「大元締め（私設ディスパッチャ）」です
。
動き方（ノンプリエンプティブ / 協調的マルチタスク）: イベントループは強制的に主導権を奪えません。その代わり、コルーチン（作業員）たちが**「今は通信待ちで暇だから、CPUの主導権を返すよ（await）」と自主的に宣言する**ことで、次のコルーチンに作業を割り振ります。
第2部：【解剖】 どう連携しているのか？（スローモーション）
では、OSとPythonのイベントループがどう連携して、あの「圧倒的な並行処理」を実現しているのか、階層構造を見てみましょう。
OSのディスパッチャ「よし、Pythonのプロセス（メインスレッド）にCPUの使用権を与えるぞ！ 走れ！」
Pythonのイベントループ「OSからCPUの許可が来たぞ！ よし、コルーチンA（ルーター1へのSSH接続）、お前動け！」
コルーチンA「ルーター1に接続要求を出しました！ 応答が来るまで暇なので、イベントループさんにCPUの主導権を返します（await）！」
Pythonのイベントループ「よし、主導権を受け取ったぞ！ OSからもらった俺たちのCPU持ち時間はまだ残っているから、すかさずコルーチンB（ルーター2へのSSH）を動かすぞ！」
コルーチンB「ルーター2に接続要求を出しました！ 主導権を返します（await）！」
(これを100台分繰り返す)
OSのディスパッチャ「おっと、Pythonの持ち時間が終わったな。一旦CPUを没収して、別のアプリ（ブラウザなど）にCPUを回すぞ。……はい、またPythonの番だ！」
Pythonのイベントループ「OSからCPUが戻ってきたぞ！ おっ、さっきのルーター1からデータが返ってきている（OSからのI/O完了通知）！ よしコルーチンA、再開しろ！」
--------------------------------------------------------------------------------
第3部：【アーキテクトの視点】 なぜOSに任せずに自分でディスパッチするのか？
昌敏さんはこう思うかもしれません。「わざわざPythonの中でイベントループを作らなくても、全部OSのディスパッチャに任せて、100個のOSスレッドを作ればいいのでは？」
実は、一昔前（マルチスレッド全盛期）はそうしていました。しかし、OSのディスパッチャは非常に重厚なため、スレッドを1万個も作ると、OSが「誰にCPUを渡すか」の管理（コンテキストスイッチ）だけでCPUを使い果たしてしまい、システムがパンクします（これを C10K問題 と呼びます）。
そこで生まれたのが、asyncio（イベントループ）のアーキテクチャです。 OSから見れば「Pythonという1つのスレッド」がずっと動いているだけなので、OSの負担は最小限です
。その「1つのスレッドの中で与えられたCPU時間」を、Pythonのイベントループがミリ秒単位で無駄なく1万個のコルーチンに細かく割り振る（await でキャッチボールする）ことで、**「OSのオーバーヘッド限界を超えた超・軽量な並行処理」**を実現しているのです！
--------------------------------------------------------------------------------
■ 参謀からのまとめ
OSのディスパッチャ ＝ 会社（OS）の社長。各部署（プロセス/スレッド）に予算（CPU時間）を強制的に割り当てる
。
イベントループ ＝ Python部署の「現場監督」。社長（OS）からもらった限られた予算（CPU時間）の中で、部下（コルーチン）がサボっている時間（I/O待ち）を見つけては、別の部下に仕事を回し、部署全体の生産性を極限まで高める
。
