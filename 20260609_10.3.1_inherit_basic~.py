...
しかし、hbp が誕生した瞬間に継承大元の Person の __init__ が実行された結果、
...
私たちが普段 super() と引数なしで使っているものは「超・省略形」です。 
実際の super クラスの初期化メソッド（__init__）は、
内部的に以下のような2つの引数を受け取るように定義されています。
...
つまり、代理人（プロキシ）として正しく機能するためには、
**「私は今どのクラスの設計図の中にいるのか（現在地）」
と「誰の代わりに仕事をするのか（依頼主）」**という
2つの絶対的なコンテキストが必要なのです。

第2部：【解剖】 引数なし super() に隠されたコンパイラの魔法
では、なぜ私たちは引数なしの super() だけで済んでいるのでしょうか？ 
公式ドキュメントには、この魔法の種明かしが次のように書かれています。
「__class__ は、メソッドが __class__ または super のいずれかを参照している場合
に、コンパイラによって作成される暗黙のクロージャー参照です。
これにより (...) super() の無引数形式がレキシカルスコープに基づいて
定義されているクラスを正確に識別することを可能にします」
、コンパイラがコードの見た目（レキシカルスコープ）からwork()『このメソッドが書かれているクラス』を
特定し、work()がそのクラスの金型（__class__）を忘れないようにカプセル化
（クロージャー参照）work() メソッドのポケット（__closure__）して、に縫い付けておこうということ。
メソッドに背負わせている。ということ。
